...
- Sprint 30: Interior del barco durante viaje (ship_interior.gd + ship_interior.tscn). View enum extendido a GPS/DUNGEON/SHIP. Toggle GPS↔ShipInterior con botón "Ver barco". Fix parpadeo Action Button (dungeon.visible=false en _on_voyage_updated). Fix ghost render ship_interior (usar disable_camera()/enable_camera() de ship_interior en lugar de buscar Camera2D por path). Fix dungeon invisible al bajar en isla destino.
- Sprint 30 fixes adicionales: Fix Supabase numeric overflow en voyages (doblones_at_departure excedía int4 — usar mini(doblones, 999999) o limpiar valor en DB). Fix punto azul GPS desaparecía durante viaje (_on_voyage_updated debe asignar player_screen_pos = _map_center después de _place_islands_relative). Fix _center_on_home_island simplificado incorrectamente por agente — debe iterar islands y asignar player_screen_pos = island.pos para isla tipo "home". Guard en _on_player_moved: if not VoyageManager.active_voyage.is_empty(): return para evitar que GPS real desplace el punto durante viaje.
